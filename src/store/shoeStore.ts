...
export const useShoeStore = create<State & Actions>((set) => ({
  shoeInCart: [],

  addShoeToCart: (shoe: Shoe) => {
    set((state) => {
      const itemInCart = state.shoeInCart.find((item) => item.id === shoe.id);
  ...
  removeShoeProductOneFromCart: (shoeId: number) => {
    set((state) => {
      const shoeToDelete = state.shoeInCart.find((item) => item.id === shoeId);
